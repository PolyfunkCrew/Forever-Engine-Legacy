...
class DialogueBox extends FlxSpriteGroup
{
	...
	public function dialoguePath(file:String):String
	{
		...
		// load the json file
		if (openfl.Assets.exists(dialoguePath))
			return dialoguePath;
		else
			return truePath;
	}
	...
	public function updateTextBox(force:Bool = false)
	{
		...
		if (curBoxState != newState || force)
		{
			...
			boxData = haxe.Json.parse(openfl.Assets.getText(boxJson));
	...
	public function updatePortrait(force:Bool = false)
	{
		...
		if (curCharacter != newChar || force)
		{
			if (newChar != null)
			{
				...
				// load the json file
				if (openfl.Assets.exists(portraitJson))
				{
					portraitData = haxe.Json.parse(openfl.Assets.getText(portraitJson));
